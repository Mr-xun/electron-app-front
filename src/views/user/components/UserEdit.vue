<template>
    <el-dialog :title="editUserTitle" :visible="editUserVisible" class="edituser-dialog" :before-close="closeEditUser"
        ...
        <el-form :model="editUserForm" label-width="80px" :rules="editUserRules" ref="editUserForm">
            ...
            <el-form-item label="用户图标">
                <el-upload class="avatar-uploader" :headers="{authorization: `Bearer ${$store.state.user.token}`}"
                    :action="`${baseUrl}/file/upload`" :show-file-list="false"
                    accept="image/png, image/gif, image/jpg, image/jpeg" :on-success="handleAvatarSuccess">
                    <el-image v-if="editUserForm.avatar" class="avatar" style="width: 100px; height: 100px"
                        :src="editUserForm.avatar" fit="cover"></el-image>
...
export default class UserEdit extends Vue {
    ...
    private baseUrl =
        process.env.NODE_ENV == "production" ? proBaseUrl : devBaseUrl;
