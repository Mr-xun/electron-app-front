<template>
  <el-dialog
    :title="editGoodsTitle"
    :visible="editGoodsVisible"
    class="editgoods-dialog"
    :before-close="closeEditGoods"
  ...
  >
    <el-form :model="editGoodsForm" label-width="150px" :rules="editGoodsRules" ref="editGoodsForm">
      ...
      <el-form-item label="商品品牌" prop="goods_brandCode">
        <el-select
          clearable
          v-model="editGoodsForm.goods_brandCode"
          placeholder="品牌"
          style="width:325px;"
        >
          <el-option
      ...
      <el-form-item label="库存">
        <el-input-number
          ...
          :min="0"
          v-model="editGoodsForm.goods_inventory"
          style="width:325px;"
        ></el-input-number>
      </el-form-item>
      <el-form-item label="规格">
        <div class="inventory-item">
          ...
          <el-input-number
            controls-position="right"
            v-model="editGoodsForm.goods_specifica"
            style="width:285px;"
          ></el-input-number>
        </div>
      </el-form-item>
      <el-form-item label="进货价">
        <el-input-number
          controls-position="right"
          :min="0"
          v-model="editGoodsForm.goods_iptPrice"
          style="width:325px;"
        ></el-input-number>
      </el-form-item>
      <el-form-item label="零售价">
        <el-input-number
          controls-position="right"
          :min="0"
          v-model="editGoodsForm.goods_optPrice"
          style="width:325px;"
        ></el-input-number>
      </el-form-item>
      <el-form-item label="批发价(通用)">
        <el-input-number
          controls-position="right"
          :min="0"
          v-model="editGoodsForm.goods_pubTradePrice"
          style="width:325px;"
        ></el-input-number>
      </el-form-item>
      <el-form-item
        :label="'商户模板'+(index+1)"
        v-for="(item,index) in editGoodsForm.goods_tradePriceTemps"
        :prop="'goods_tradePriceTemps.' + index + '.template_code'"
        :rules="{
      ...
      >
        <div class="tradePrices-item">
          <el-select
            ...
            @change="changeMerchant(item.template_code,item)"
            placeholder="商户"
            style="width:160px"
          >
            <el-option
...
export default class GoodsEdit extends Vue {
  ...
  private changeMerchant(value: string, item: any) {
    let result = BaseDataModule.merchantTypes.filter(
      (merchant: { merchant_code: string }) => merchant.merchant_code == value
    );
    item.template_name = result[0].merchant_name;
  }
  private submitGoods() {
    let params: any = this.editGoodsForm;
    (this.$refs["editGoodsForm"] as any).validate(async (valid: boolean) => {
      if (valid) {
        try {
          if (this.editGoodsId) {
